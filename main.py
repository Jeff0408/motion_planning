...
def runtest_Astar(mapfile, start, goal, verbose = True):
    ...
    boundary, blocks = load_map(mapfile)
    PR = Planner.MyPlanner(boundary, blocks)
    # Display the environment
    if verbose:
        fig, ax, hb, hs, hg = draw_map(boundary, blocks, start, goal)  
    ...
    while True:
        ...
        t0 = tic()
        newrobotpos = PR.Astar(robotpos, goal)
       ...
       # Check if the commanded position is valid
        ...
        path = np.array(pos)
        pathlength = np.sum(np.sqrt(np.sum(np.diff(path,axis=0)**2,axis=1)))
        #print('pos', robotpos[0])
        #print('pos', robotpos[1])
        #print('pos', robotpos[2])
...
def test_single_cube(verbose = False):
  ...
  success, pathlength = runtest_Astar('./maps/single_cube.txt', start, goal, verbose)
...
def test_window(verbose = False):
  ...
  success, pathlength = runtest_Astar('./maps/window.txt', start, goal, verbose)
  print('Success: %r'%success)
  print('Path length: %d'%pathlength)
  print('\n')
...
def test_monza(verbose = False):
  ...
  success, pathlength = runtest_Astar('./maps/monza.txt', start, goal, verbose)
  print('Success: %r'%success)
  print('Path length: %d'%pathlength)
  print('\n')
